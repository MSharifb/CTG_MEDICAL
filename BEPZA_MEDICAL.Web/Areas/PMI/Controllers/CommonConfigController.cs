...
namespace BEPZA_MEDICAL.Web.Areas.PMI.Controllers
{
    public class CommonConfigController : Controller
    {
        ...
        #region Ctor
        public CommonConfigController(PMICommonService invCommonService, PRMCommonSevice prmCommonservice)
        {
            _pmiCommonservice = invCommonService;
        ...
        {
            List<BEPZA_MEDICAL.DAL.PMI.CommonConfigType> list = _pmiCommonservice.PMIUnit.ConfigTypeRepository.GetAll().OrderBy(x => x.DisplayName).ToList();
            list = list.Where(d => d.ModuleId == (int)ApplicationModule.PMI).OrderBy(q => q.SortOrder).ToList();
