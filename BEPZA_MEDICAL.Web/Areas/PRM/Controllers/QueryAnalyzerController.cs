...
{
    public class QueryAnalyzerController : BaseController
    {
        #region Fields
        ...
        public QueryAnalyzerController(PRMCommonSevice pPRMCommonService, EmployeeService empService)
        {
       ...
       // [HttpPost]
        //[ValidateInput(false)]
        public ActionResult Preview(QueryAnalyzerViewModel model)
        {
            if (!ModelState.IsValid)
            {
                return RedirectToAction("Index");
                    ...
                    if (item.SortType != null)
                        if (item.SortType.Length > 0)
                        {
                            if (item.SortType == "Ascending")
                                ...
                                sortBy = " Desc ";
                            if (type == "date")
                                sortColumn += "convert(datetime,[" + item.ItemName + "],105),";
                            else
