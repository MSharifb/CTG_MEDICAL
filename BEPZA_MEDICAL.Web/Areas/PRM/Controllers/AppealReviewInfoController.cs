...
{
    public class AppealReviewInfoController : BaseController
    {
        #region Fields
        ...
        #region Ctor
        public AppealReviewInfoController(PRMCommonSevice prmCommonService)
        {
        ...
        public ActionResult GetList(JqGridRequest request, AppealReviewInfoViewModel model)
        {
            string filterExpression = String.Empty;
            int totalRecords = 0;
            List<AppealReviewInfoViewModel> list = (from appealReview in _prmCommonService.PRMUnit.AppealReviewInfoRepository.GetAll()
                                                    join comNoteSheet in _prmCommonService.PRMUnit.ComplaintNoteSheetRepository.GetAll() on appealReview.ComplaintNoteSheetId equals comNoteSheet.Id
                                                    ...
                                                    && (model.AppealDate == null || model.AppealDate == appealReview.AppealDate)

                                                    select new AppealReviewInfoViewModel()
                                                 {
                                                     Id = appealReview.Id,
                                                     ComplaintNoteSheetId = appealReview.ComplaintNoteSheetId,
                                                     ComplaintNoteSheetName = appealReview.PRM_ComplaintNoteSheet.DeptProceedingNo,
                                                     AppealDate = appealReview.AppealDate,
                                                 }).OrderBy(x => x.AppealDate).ToList();
            ...
            if (request.SortingName == "AppealDate")
            {
                if (request.SortingOrder.ToString().ToLower() == "asc")
                {
                    list = list.OrderBy(x => x.AppealDate).ToList();
                }
                else
                {
                    list = list.OrderByDescending(x => x.AppealDate).ToList();
                }
            }
            #endregion
            ...
            foreach (var d in list)
            {
                ...
                response.Records.Add(new JqGridRecord(Convert.ToString(d.Id), new List<object>()
                {
                    ...
                    Convert.ToDateTime(d.AppealDate).ToString("dd-MM-yyyy"),                  
                    "Delete"
                }));
            }
        ...
        {
            AppealReviewInfoViewModel model = new AppealReviewInfoViewModel();
        ...
        [HttpPost]
        public ActionResult Create([Bind(Exclude = "Attachment")] AppealReviewInfoViewModel model)
        {
            try
            {
                string errorList = "";
                ...
                if (ModelState.IsValid)
                {
                    ...
                    {
                        _prmCommonService.PRMUnit.AppealReviewInfoRepository.Add(entity);
                        _prmCommonService.PRMUnit.AppealReviewInfoRepository.SaveChanges();
                        model.IsError = 0;
        ...
        {
            var entity = _prmCommonService.PRMUnit.AppealReviewInfoRepository.GetByID(Id);
            var model = entity.ToModel();
            model.ComplaintNoteSheetName = entity.PRM_ComplaintNoteSheet.DeptProceedingNo;
            model.strMode = "Edit";

            model.ApplyByEmpId = entity.PRM_EmploymentInfo.EmpID;
            model.ApplyByEmployeeName = entity.PRM_EmploymentInfo.FullName;
            model.ApplyByDesignationName = entity.PRM_EmploymentInfo.PRM_Designation.Name;
        ...
        [HttpPost]
        [NoCache]
        public ActionResult Edit([Bind(Exclude = "Attachment")] AppealReviewInfoViewModel model)
        {
            try
            {
                string errorList = "";
                ...
                if (ModelState.IsValid)
                {
                    ...
                    var obj = _prmCommonService.PRMUnit.AppealReviewInfoRepository.GetByID(model.Id);
                    ...
                    if (errorList.Length == 0)
                    {
                        _prmCommonService.PRMUnit.AppealReviewInfoRepository.Update(entity);
                        _prmCommonService.PRMUnit.AppealReviewInfoRepository.SaveChanges();
                        model.errClass = "success";
        ...
        public JsonResult DeleteConfirmed(int id)
        {
            ...
            try
            {
                _prmCommonService.PRMUnit.AppealReviewInfoRepository.Delete(id);
                _prmCommonService.PRMUnit.AppealReviewInfoRepository.SaveChanges();
                result = true;
        ...
        private void populateDropdown(AppealReviewInfoViewModel model)
        {
            dynamic ddlList;
        ...
        private AppealReviewInfoViewModel GetInsertUserAuditInfo(AppealReviewInfoViewModel model, bool pAddEdit)
        {
            if (pAddEdit)
            {
        ...
        private int Upload(AppealReviewInfoViewModel model)
        {
            if (model.File == null)
                return 0;
        ...
        public void DownloadDoc(AppealReviewInfoViewModel model)
        {
        ...
        {
            var ddlList = (from aplRv in _prmCommonService.PRMUnit.AppealReviewInfoRepository.GetAll().GroupBy(d => d.ComplaintNoteSheetId).Select(grp => grp.First()).ToList()
                           join comNoteSheet in _prmCommonService.PRMUnit.ComplaintNoteSheetRepository.GetAll().Where(q => q.ZoneInfoId == LoggedUserZoneInfoId) on aplRv.ComplaintNoteSheetId equals comNoteSheet.Id
                           select comNoteSheet).OrderBy(x => x.DeptProceedingNo).ToList();
