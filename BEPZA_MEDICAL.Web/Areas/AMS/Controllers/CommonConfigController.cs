...
namespace BEPZA_MEDICAL.Web.Areas.AMS.Controllers
{
    public class CommonConfigController : Controller
    {
        ...
        #region Ctor
        public CommonConfigController(AMSCommonService amsCommonService, PRMCommonSevice prmCommonservice)
        {
        ...
        {
            List<BEPZA_MEDICAL.DAL.AMS.CommonConfigType> list = _amsCommonservice.AMSUnit.ConfigTypeRepository.GetAll().OrderBy(x => x.DisplayName).ToList();
            list = list.Where(d => d.ModuleId == (int)ApplicationModule.AMS).OrderBy(q => q.SortOrder).ToList();
