...
{
    public partial class RptEmployeeTrainingInfo : ReportBase
    {
        #region Fields
        #endregion
        ...
        protected void btnViewReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int tariningTypeId=Convert.ToInt32(ddlTrainingType.SelectedValue);
                ...
                GenerateReport(tariningTypeId, employeeId, strZoneId,trainingTitle,organizedBy,fundedby,countryId,departmentId,fromDate,toDate);
            }
            catch (Exception ex)
            {
        ...
        public void GenerateReport(int tariningTypeId, int employeeId, string strZoneId, string trainingTitle, string organizedBy, string fundedby, int countryId, int departmentId, DateTime? fromdate, DateTime? toDate)
        {
            #region Processing Report Data

            rvTestResult.Reset();
            rvTestResult.ProcessingMode = ProcessingMode.Local;
            rvTestResult.LocalReport.ReportPath = Server.MapPath("~/Reports/PRM/rdlc/EmployeeTrainingInfo.rdlc");

            var data = (from s in base.prmContext.SP_PRM_RptEmpyoleeTrainingInfo(tariningTypeId, employeeId, strZoneId, trainingTitle,organizedBy,fundedby,countryId, departmentId, fromdate, toDate) select s).ToList();

            #region Search parameter


            string searchParameters1 = string.Empty;
            searchParameters1 = "Training Type:  " + ddlTrainingType.SelectedItem.Text; ;

            ReportParameter p1 = new ReportParameter("p1", searchParameters1);
        ...
        private void PopulateDropdownList()
        {
            ...
            ddlTrainingType.DataSource = context.PRM_TrainingType.OrderBy(x => x.Name).ToList();
            ddlTrainingType.DataValueField = "Id";
            ddlTrainingType.DataTextField = "Name";
            ddlTrainingType.DataBind();
            ddlTrainingType.Items.Insert(0, new ListItem("All", "0"));

            ddlTrainingTitle.DataSource = context.PRM_EmpTrainingInfo.DistinctBy(s => s.TrainingTitle).OrderBy(x=>x.TrainingTitle).ToList();
            ddlTrainingTitle.DataValueField = "TrainingTitle";
            ddlTrainingTitle.DataTextField = "TrainingTitle";
            ddlTrainingTitle.DataBind();
            ddlTrainingTitle.Items.Insert(0, new ListItem("All", "0"));

            ddlOrganizedBy.DataSource = context.PRM_EmpTrainingInfo.OrderBy(x => x.OrganizedBy).ToList();
            ...
            ddlFundedBy.DataSource = context.PRM_EmpTrainingInfo.OrderBy(x => x.Financeby).ToList();
            ddlFundedBy.DataValueField = "Financeby";
            ddlFundedBy.DataTextField = "Financeby";
