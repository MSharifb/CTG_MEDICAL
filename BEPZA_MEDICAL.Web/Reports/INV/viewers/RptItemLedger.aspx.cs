...
{
    public partial class RptItemLedger : ReportBase
    {
        #region Fields
        ...
        protected void btnViewReport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                GenerateReport(strZoneId, dateFrom, dateTo, itemTypeId, categoryId, colorId, modelId, itemId, assetGroupId);
        ...
        public void GenerateReport(string strZoneId, DateTime dateFrom, DateTime dateTo, int itemTypeId, int categoryId, int colorId, int modelId, int itemId, int assetGroupId)
        {
            ...
            rvItemLedger.Reset();
            rvItemLedger.ProcessingMode = ProcessingMode.Local;
            rvItemLedger.LocalReport.ReportPath = Server.MapPath("~/Reports/INV/rdlc/RptItemLedger.rdlc");

            var data = invContext.sp_INV_RptItemLedger(strZoneId, dateFrom, dateTo, itemTypeId, categoryId, modelId, colorId, itemId, assetGroupId, numErrorCode, strErrorMsg).ToList();
            ...
            if (data.Count() > 0)
            {
                ...
                searchParameters = "Period From : " + dateFrom.ToString("dd-MMM-yyyy") + " To " + dateTo.ToString("dd-MMM-yyyy");
                ReportParameter p1 = new ReportParameter("param", searchParameters);
                rvItemLedger.LocalReport.SetParameters(new ReportParameter[] { p1 });

                #endregion


                ReportDataSource dataSource = new ReportDataSource("dsItemLedger", data);
                rvItemLedger.LocalReport.DataSources.Add(dataSource);
                this.rvItemLedger.LocalReport.SubreportProcessing += new SubreportProcessingEventHandler(localReport_SubreportProcessing);
            }
            else
            {
                checkStatus = true;
            ...
            rvItemLedger.DataBind();

            //ExportToPDF
            String newFileName = "ItemLedger_" + Guid.NewGuid() + ".pdf";
            String newFilePath = "~/Content/TempFiles/" + newFileName;
            FileStream fs = new FileStream(Server.MapPath(newFilePath), FileMode.Create);
            BEPZA_MEDICAL.Web.Utility.Common.ExportPDF(rvItemLedger, newFileName, fs);
        ...
        protected void rvItemLedger_ReportRefresh(object sender, CancelEventArgs e)
        {
