...
{
    public partial class RptZonewiseWelfareFund : ReportBase
    {
        #region Fields
        ...
        protected void btnViewReport_Click(object sender, EventArgs e)
        {
            try
            {
               ...
               var refNo = ddlRefNo.SelectedValue.ToString();
               GenerateReport(strZoneId, empId, year, refNo);
            }
            catch (Exception ex)
            {
        ...
        public void GenerateReport(string strZoneList, string empId, string year, string refNo)
        {
            #region Processing Report Data

            rvEmployeeInfo.Reset();
            rvEmployeeInfo.ProcessingMode = ProcessingMode.Local;
            rvEmployeeInfo.LocalReport.ReportPath = Server.MapPath("~/Reports/WFM/rdlc/ZonewiseWelfareFund.rdlc");

            var data = (from s in base.wfmContext.SP_WFM_ZoneWiseWelfareFund(strZoneList, empId, year, refNo) select s).ToList();
            ...
            if (refNo == "")
            {
                searchParameters = "Ref. No. : All";
            }
            else
            {
                searchParameters = "Ref. No. : "+ddlRefNo.SelectedItem.Text;
            }
            ...
            ReportDataSource dataSource = new ReportDataSource("DSZonewiseWelfareFund", data);
            ...
            //ExportToPDF
            String newFileName = "ZoneWiseWelfareFundReport_" + Guid.NewGuid() + ".pdf";
        ...
        private void PopulateDropdownList()
        {
            ...
            ddlRefNo.DataSource = wfmContext.WFM_ApprovalWelfareFundInfo.OrderBy(x => x.RefNo).ToList();
            ddlRefNo.DataValueField = "RefNo";
            ddlRefNo.DataTextField = "RefNo";
            ddlRefNo.DataBind();
            ddlRefNo.Items.Insert(0, new ListItem("All", ""));
